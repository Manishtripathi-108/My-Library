...
<nav class="bg-light-secondary dark:bg-dark-secondary" x-data="{ open: false }">
	...
	<div class="hidden sm:hidden" :class="{ 'block': open, 'hidden': !open }">
		...
		<div class="border-t border-gray-200 pb-1 pt-4 dark:border-gray-600">
			<div class="px-4">
				<div class="text-base font-medium text-gray-800 dark:text-dark-text-secondary" x-data="{{ json_encode(['name' => auth()->user()->name]) }}" x-text="name" x-on:profile-updated.window="name = $event.detail.name"></div>
				<div class="text-sm font-medium text-light-text-secondary">{{ auth()->user()->email }}</div>
			</div>
